...
public class LGBMBooster implements AutoCloseable {
    ...
    public synchronized static void loadNative() throws IOException {
        if (!nativeLoaded) {
            ...
            if (os.startsWith("Linux") || os.startsWith("LINUX")) {
                loadNative("linux/x86_64/lib_lightgbm.so", "linux/x86_64/lib_lightgbm.so");
                loadNative("linux/x86_64/lib_lightgbm_swig.so", "linux/x86_64/lib_lightgbm_swig.so");
                nativeLoaded = true;
    ...
    public String saveModelToJson(int startIteration, int numIteration, FeatureImportanceType featureImportance) {
        SWIGTYPE_p_long_long outLength = new_int64_tp();
        String result = LGBM_BoosterDumpModelSWIG(
                voidpp_value(handle),
                startIteration,
                numIteration,
                importanceType(featureImportance),
    ...
    public String saveModelToJson(int numIteration) {
        return saveModelToJson(0, numIteration, FeatureImportanceType.SPLIT);
    }
    /**
